...
| `# 标题` / `## 标题` / `### 标题` | 二/三/四级标题 |
| `![描述](图片URL)` | 独立一行图片（懒加载） |
| `![描述](视频URL)` | URL 以 `.mp4/.webm/.mov` 结尾时自动渲染为内嵌 `<video>`（带播放控件） |
| `- 项目` / `* 项目` | 无序列表 |
| `**加粗**` | 加粗 |
| `[文字](URL)` | 链接（新窗口打开） |
...
front matter 加 `gallery: true` 时，正文图片切换为等宽瓦片网格（4:3 裁切、hover 微放大）；标题与段落仍跨整行。适合图集类项目（如 g6 项目图集）。
...
- **配图**：正文 `![]()` 引用本地路径，弹窗内以原比例完整显示（上限 460px 高、不裁剪）。多图项目（如 g5 毕业作品集、g6 项目图集）图片按项目存放到子目录：`assets/images/projects/<id>/`，md 内引用 `assets/images/projects/<id>/文件名`
